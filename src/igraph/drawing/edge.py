...
class AbstractEdgeDrawer(object):
    ...
    def __init__(self, context, palette):
        ...
        @param context: a Cairo context or Matplotlib Axes on which
                        the edges will be drawn.
...
class ArrowEdgeDrawer(AbstractEdgeDrawer):
    ...
    def intersect_bezier_circle(
        ...
        source_target_distance = cls.euclidean_distance(x0, y0, x3, y3)
        radius = float(radius)
        t0 = 1.0
        t1 = 1.0 - radius / source_target_distance

        xt1, yt1 = cls.bezier_cubic(x0, y0, x1, y1, x2, y2, x3, y3, t1)

        distance_t0 = 0
        distance_t1 = cls.euclidean_distance(x3, y3, xt1, yt1)
        counter = 0
        while abs(distance_t1 - radius) > precision and counter < max_iter:
            ...
            distance_t1 = cls.euclidean_distance(x3, y3, xt1, yt1)
            counter += 1
        return cls.bezier_cubic(x0, y0, x1, y1, x2, y2, x3, y3, t1)
    ...
    def draw_directed_edge(self, edge, src_vertex, dest_vertex):
        ...
        if edge.curved:
            ...
            x2, y2 = self.intersect_bezier_circle(
            ...
            offset_length /= (
                self.euclidean_distance(0, 0, x_arrow_base_vec, y_arrow_base_vec) ** 2
            )
...
class MatplotlibArrowEdgeDrawer(MatplotlibEdgeDrawer, ArrowEdgeDrawer):
    def draw_directed_edge(self, edge, src_vertex, dest_vertex):
        if src_vertex == dest_vertex:  # TODO
            return self.draw_loop_edge(edge, src_vertex)
