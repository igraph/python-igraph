...
PyObject *igraphmodule_Graph_chordal_completion(
  ...
  igraph_vector_t alpha, alpham1, edges;
  igraph_vector_t *alpha_ptr = 0, *alpham1_ptr = 0;
  ...
  if (alpha_o != Py_None) {
    if (igraphmodule_PyObject_to_vector_t(alpha_o, &alpha, IGRAPHMODULE_TYPE_INT)) {
      return NULL;
    }
  ...
  if (alpham1_o != Py_None) {
    if (igraphmodule_PyObject_to_vector_t(alpham1_o, &alpham1, IGRAPHMODULE_TYPE_INT)) {
      if (alpha_ptr) {
        igraph_vector_destroy(alpha_ptr);
      }
      return NULL;
    }
  ...
  if (igraph_vector_init(&edges, 0)) {
    igraphmodule_handle_igraph_error();
    if (alpham1_ptr) {
      igraph_vector_destroy(alpham1_ptr);
    }
    if (alpha_ptr) {
      igraph_vector_destroy(alpha_ptr);
    }
  ...
  res_o = igraphmodule_vector_t_to_PyList_pairs(&edges);
  igraph_vector_destroy(&edges);
...
PyObject *igraphmodule_Graph_maximum_cardinality_search(igraphmodule_GraphObject *self) {
  ...
  if (igraph_vector_init(&alpha, 0)) {
    igraphmodule_handle_igraph_error();
    return NULL;
  }
  
  if (igraph_vector_init(&alpham1, 0)) {
  ...
  alpha_o = igraphmodule_vector_t_to_PyList(&alpha, IGRAPHMODULE_TYPE_INT);
  igraph_vector_destroy(&alpha);
  ...
  alpham1_o = igraphmodule_vector_t_to_PyList(&alpham1, IGRAPHMODULE_TYPE_INT);
  igraph_vector_destroy(&alpham1);
...
struct PyMethodDef igraphmodule_Graph_methods[] = {
  ...
  {"contract_vertices", (PyCFunction) igraphmodule_Graph_contract_vertices,
   ...
   "Contracts some vertices in the graph, i.e. edges to and from a group of\n"
   "vertices will then point to a single vertex. Edges are not combined.\n\n"
   "This operation does not delete vertices or edges. Call L{simplify()} or\n"
   "L{delete_vertices()} as needed.\n\n"
   "@param mapping: list of new vertex IDs the current IDs should be mapped\n"
   "  onto. C{mapping} therefore needs to have an entry for every vertex in\n"
   "  the graph. Vertices having the same new vertex ID in this vector will\n"
   "  be remapped into a single new vertex. It is safe to pass the\n"
